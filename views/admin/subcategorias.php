    ...
    <div class="admin-container">
        <div class="card">
            <div>
                ...
                <div class="cardName">Subcategorias</div>
            </div>
        ...
        <div class="admin-form">
            <form class="admin-form__form" id="formProductos" action="" method="POST" enctype="multipart/form-data">
                <h4>Agregar Subcategorias</h4>
                <label for="nombre">Nombre:
                    <input type="text" placeholder="Nombre" name="nombre_subcat"
                        value="<?= $id->nombre_subcat ?? null ?>">
                </label>
                <?= isset($errores['nombreVacio']) ? $errores['nombreVacio'] : '';  ?>
                <label for="categoria">Categoria:
                    <select name="id_categoria" id="">
                        <?php foreach ($categorias as $cat) : ?>
                        <option value="<?= $cat->id_categoria; ?>" <?php if (isset($id->id_categoria)) : ?>
                            <?= $cat->id_categoria === $id->id_categoria  ?  "selected" : ''; ?> <?php endif; ?>>
                            <?= $cat->nombre_categoria; ?></option>
                        <?php endforeach;  ?>
                    </select>
                </label>
                <label class=" lbl__imagen" for="imagen">Imagen:
                    <input type="file" id="imagen" name="imagen_subcat">
                </label>
                <?= isset($id->imagen_subcat) ? "<img src='../build/img/subcategorias/$id->imagen_subcat'
                width='100px'>" : '' ?>
        ...
        <div class="tablas">
            ...
            <table>
                ...
                <tbody id="tbody">
                    ...
                    <tr data-id="<?= $sub['id_subcat']; ?>">
                        <td><?= $sub['id_subcat']; ?></td>
                        <td><?= $sub['nombre_subcat']; ?></td>
                        <td><img src="../build/img/subcategorias/<?= $sub['imagen_subcat']; ?>" width="50"></td>
                        <td><?= $sub['nombre_categoria']; ?></td>
                        <td>
                            <a href="/admin/subcategorias?actualizar=<?= $sub['id_subcat']; ?>"
